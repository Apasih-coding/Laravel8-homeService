...
class AdminEditServiceComponent extends Component
{
    ...
    public function update($fields)
    {
        ...
        if ($this->new_thumbnail) {
            $this->validateOnly($fields, [
                'new_thumbnail' => 'required|mimes:jpeg,png'
            ]);
        }
        if ($this->new_image) {
            $this->validateOnly($fields, [
    ...
    public function updateService()
    {
        ...
        if ($this->new_thumbnail) {
            ...
            $imageName = Carbon::now()->timestamp . '.' . $this->new_thumbnail->extension();
            $this->new_thumbnail->storeAs('services/thumbnails', $imageName);
            $service->thumbnail = $imageName;
        }
        if ($this->new_image) {
            unlink('images/services' . '/' . $service->image);
            $imageName2 = Carbon::now()->timestamp . '.' . $this->new_image->extension();
            $this->new_image->storeAs('services', $imageName2);
    ...
    public function render()
    {
        ...
        return view('livewire.admin.admin-edit-service-component', ['categories' => $categories])->layout('layouts.main');
    }
}
